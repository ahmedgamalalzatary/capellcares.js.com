...
describe("buildNav", () => {
  it("builds recursive nav nodes for arbitrarily deep category trees", () => {
    ...
    expect(nav[0].root.id).toBe(1);
    expect(nav[0].children.map((child) => child.id)).toEqual([2, 5]);
    expect(nav[0].children[0].children[0].id).toBe(3);
    expect(nav[0].children[0].children[0].children[0].id).toBe(4);
  });
});
