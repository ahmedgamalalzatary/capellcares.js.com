...
describe("storefront api client", () => {
  ...
  it("does not retry the token that just received a 401 when refresh is unavailable", async () => {
    ...
    await expect(fetchCustomerOrders("expired-token")).rejects.toThrow("Authentication refresh unavailable");
    expect(fetch).toHaveBeenCalledTimes(2);
  });

  it("distinguishes a transient refresh failure from a rejected session", async () => {
    vi.stubGlobal("fetch", vi.fn()
  ...
  it("discards a successful authenticated checkout response after the account changes", async () => {
    let releaseCheckout!: (response: unknown) => void;
    const firstCheckout = new Promise((resolve) => { releaseCheckout = resolve; });
    vi.stubGlobal("fetch", vi.fn()
      .mockImplementationOnce(() => firstCheckout)
    ...
    releaseCheckout({
      ok: true,
      status: 201,
      json: async () => ({ id: 8, orderCode: "ORDER-A", paymentStatus: "pending" })
    });
