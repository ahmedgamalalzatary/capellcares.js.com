...
export function OfferGrid({
  ...
  const bigCategories = useMemo<Category[]>(() => {
    ...
    for (const offer of offers) {
      if (offer.categoryId == null) continue;
      const root = findRootCategory(activeCategories, offer.categoryId);
      if (root) {
        rootsWithOffers.set(root.id, root);
      }
    }
    return [...rootsWithOffers.values()].sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
  }, [activeCategories, offers]);
  ...
  const filtered = useMemo(() => {
    ...
    const next = selectedIds.length === 0
      ...
      : offers.filter((offer) =>
        ...
        selectedIds.some((selectedId) => isInCategoryTree(activeCategories, offer.categoryId!, selectedId))
      );
  ...
  }, [activeCategories, categoryId, pillCategoryIds, offers, lang, sort]);

  const hasActiveFilters = categoryId != null || pillCategoryIds.length > 0;

  if (offers.length === 0) {
    return <p className="py-12 text-center text-(--ink-3)">{dict.offers.listEmpty}</p>;
  }
