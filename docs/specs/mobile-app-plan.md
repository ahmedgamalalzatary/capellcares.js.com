...
| `apps/mobile/src/lib/api/http.ts` | `getJSON` / `authedGetJSON` / `authedMutationJSON` with 401 → refresh → retry-once logic and `x-lang` from the caller's current language state. The Phase 4 auth adapter supplies a session revision so a delayed request can never retry under a different signed-in customer, and an already-rotated access token is reused without another refresh. Checkout retries only a received 401: that response is emitted by auth middleware before checkout validation or order creation, so the retry cannot duplicate an order. Timeouts and other ambiguous failures are never retried |
